...
- Centralize brand assets in `config/brand.ts` (light/dark logos, hero video/poster, OG image, favicon) with helper `getActiveLogo`.
- Fix language toggle to show target language; complete EN/AR translations; safer array access in translations.
...
- Visual polish across Industries, Services, Products, About, Register, Quote pages with imagery, hover/animation, and improved forms/accessibility.
- Update metadata to use brand config for Open Graph and icon.
- Replace all remaining `<img>` with `next/image` and add blur placeholders (`lib/blur.ts`) for better LCP.
...
- `components/*Card.tsx` (images + interactions)
